...
## 후속 처리 박제 (cycle 432, 2026-05-15)

cycle 432 info-architecture-review chain (heavy, 30-cycle 주기 보정 trigger) 진단 결과 잔존 1건 close 결정:

| 후속 | 결정 | 근거 |
|---|---|---|
| 커뮤니티 그룹에 `/search` 또는 소셜 링크 추가 | **진행 X (close)** | `/search` 는 이미 utility 로 노출 — `NavLinks.tsx:155` 데스크탑 nav 끝 `<SearchForm compact />` + `Header.tsx:67` 모바일 헤더 검색 아이콘. 카테고리 그룹 안 진입은 멘탈 모델 X (검색 = 카테고리 X, utility). 소셜 링크 = 외부 이탈 (부정 신호). 추가 가치 부재. |

본 spec **전면 close** — trigger 8 (carry-over 미처리 ≥ 20 사이클) 본 spec 더 이상 발화 X. 모든 carry-over 처리 완료.
