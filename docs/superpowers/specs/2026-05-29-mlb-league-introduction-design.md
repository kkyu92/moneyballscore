...
- 후속 별도 plan: 인증 layer (Google + Kakao + 이메일 Magic Link, Supabase Auth)
- 후속 별도 plan: 알림 연결 layer (Telegram opt-in, Login Widget + Edge Function)
- 후속 별도 plan: community (UGC 게시판, 인증 layer 의존)
...
2026-08~09 ── n=150 도달 + escalation 검증 + 인증 layer 별도 plan ship
...
│   ├── /community              ← placeholder (후속 별도 plan)
...
| 페이지 | 책임 |
|---|---|
| `/login` | "📌 회원 기능 박제 중 — 2026-08~09 ship 예정 (postseason 직전). 후속 별도 plan 박제." |
| `/settings` | placeholder (인증 layer 후속) |
| `/community` | placeholder (community 후속 별도 plan) |
...
후속 별도 plan (인증 layer ship 시) 사용자 영역 작업:
