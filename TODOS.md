...
- **pitcher-snapshot Cloudflare 분기 통합** — `'37 * * * *'` 분기 내 UTC 토요일 15h 조건 추가. cron 슬롯 4/5 유지 (`03a4867` 정신 보존). `pitcher-snapshot.yml` schedule 키 제거는 사용자 직접 필요 (GH App workflows 권한 부재).

#### ⏸️ 남은 2건 backlog (우선순위 순)

| # | Workflow | Cron | 빈도 | ROI | 작업량 추정 | 우려 |
|---|---|---|---|---|---|---|
| 1 | `sync-batter-stats.yml` | `0 3 * * *` | 매일 1회 | ⭐ Vercel /api 호출이면 쉬움 | ~15분 | endpoint 형태 확인 필요 |
| 2 | `pat-expiry-check.yml` | `0 0 1 * *` | 매월 1일 | △ GH PAT 검사라 GH 안에서 도는 게 의미 있을 수도 | 검토 우선 | GH 안 도는 게 본질 검토 |

#### 검증 path (각 이관 시)

1. worker.ts 분기 추가 (기존 cron 분기 안 조건 통합 방식 선호 — 슬롯 절약)
2. `pnpm run deploy` (cloudflare-worker)
3. 자연 fire 1 cycle 검증 (Cloudflare logs + Supabase row 박힘 확인)
4. GH schedule 키 제거 + workflow_dispatch 보존 (사용자 직접 또는 PAT로 workflow 권한 있는 별도 push)
5. commit
