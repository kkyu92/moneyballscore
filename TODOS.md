...
5. ⏳ **N≥10 누적 OOS** (자연 누적, ~07-11 ETA) — 현재 **N=5** (1224~1228). 모두 256/256 PASS. 메모리 `content-architecture-lotto-rule-oos-sample-floor.md` floor = N=5 preliminary / N=10 actionable. 매주 토 추첨 후 자연 누적 → N=10 도달 시 256 rules 통계적 우위 재평가 + score 모델 sum 가중치 튜닝 evidence 누적 (1등 catch 0/5 → N=10 시점 0/10 if 패턴 유지 = 튜닝 GO).
