...
- **sync-batter-stats Cloudflare 이관** (PR #21 open — agent-loop fire 1/10). worker.ts `"0 3 * * *"` 분기 추가. GH schedule 키 제거. cron 슬롯 4/5.
- **pitcher-snapshot Cloudflare 이관** (PR agent-loop fire 2/10). worker.ts `"0 15 * * 6"` 분기 + `runPitcherSnapshot()` 추가. cron 슬롯 5/5 (free tier 상한 도달). ⚠️ GH App `workflows` 권한 없어 `.github/workflows/pitcher-snapshot.yml` schedule 키 제거는 사용자 직접 필요 (PR 머지 후 수동 삭제). PR 머지까지 매주 토 중복 fire (무해함).

#### ⏸️ 남은 1건 backlog (검토 필요)

| # | Workflow | Cron | 빈도 | ROI | 작업량 추정 | 우려 |
|---|---|---|---|---|---|---|
| 3 | `pat-expiry-check.yml` | `0 0 1 * *` | 매월 1일 | △ GH PAT 검사라 GH 안에서 도는 게 의미 있을 수도 | 검토 우선 | GH 안 도는 게 본질. cron 슬롯 5/5 이라 추가 불가 |

> **cron 슬롯 5/5 도달**: `pat-expiry-check.yml` 는 GH 안에서 도는 것이 자연스럽고 (GitHub API 접근), 슬롯도 없어 Cloudflare 이관 스킵 결정.
