...
#### ⏸️ 남은 3건 backlog (우선순위 순)

| # | Workflow | Cron | 빈도 | ROI | 작업량 추정 | 우려 |
|---|---|---|---|---|---|---|
| 1 | `sync-batter-stats.yml` | `0 3 * * *` | 매일 1회 | ⭐ Vercel /api 호출이면 쉬움 | ~15분 | endpoint 형태 확인 필요 |
| 2 | `pitcher-snapshot.yml` | `0 15 * * 6` | 매주 토 | ⭐ 빈도 낮음 | ~15분 | 동일 |
| 3 | `pat-expiry-check.yml` | `0 0 1 * *` | 매월 1일 | △ GH PAT 검사라 GH 안에서 도는 게 의미 있을 수도 | 검토 우선 | GH 안 도는 게 본질 검토 |
