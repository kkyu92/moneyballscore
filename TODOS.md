...
## 🟢 SUCCESS — review-code(heavy): export-but-unused heuristic lotto 스코프 6건 (cycle 2864, 2026-09-04)
...
apps/moneyball/src/lib/lotto/ 스코프 exported type/interface 10개 grep → LottoData/LottoScoreBacktest/LottoSet/LottoResult 4개만 cross-file import 확인되어 유지, 나머지 6개 0-external-hit 후보. general-purpose subagent 독립 재검증(전체 repo 재grep + barrel/index.ts 부재 확인 + co-located 테스트 cross-file import 구분 + scripts/lotto.ts import 여부 확인) — 6/6 CONFIRMED_UNUSED, false positive 0건. export 키워드만 제거: WinnerEntry / ArchiveContent / ArchiveVariant / ArchiveVariants / LottoPicks / LottoBallColor.
...
다음 사이클 추천 = review-code(heavy) 계속 — accuracy/lotto-lib 처리 완료 후 잔여 스코프 재조사(전체 lib/ 서브디렉토리 exported type/interface count 재측정) 결과 analysis(8)/leaderboard(7)/picks(8)/predictions(3)/v2-shadow-monitor(3) 5개 스코프 남음. 다음 사이클 = analysis 또는 leaderboard/picks(후보 수 많은 순) 우선 권장.

## 🟢 SUCCESS — review-code(heavy): export-but-unused heuristic standings 스코프 6건 (cycle 2863, 2026-09-04)
...
apps/moneyball/src/lib/standings/ 스코프 exported type/interface 8개 grep → EloDataPoint(EloTrendChart.tsx 컴포넌트 prop 실사용) + TeamAccuracyRow(teams/page.tsx cross-file import) 2개만 유지, 나머지 6개 0-external-hit 후보(StandingRow 는 buildStandings.ts 의 dead 재-export — 실제 소비자는 @moneyball/kbo-data 원본에서 직접 import). general-purpose subagent 독립 재검증(전체 repo 재grep + barrel/index.ts 부재 확인 + co-located 테스트 cross-file import 구분 + packages/ 동명 무관 심볼 확인) — 6/6 CONFIRMED_UNUSED, false positive 0건. export 키워드만 제거(또는 재-export 라인 삭제): StandingRow / EloTrendData / MagicNumberTeam / MatchupRow / TeamHomeAwayRow / TeamBiasRow.
...
다음 사이클 추천 = review-code(heavy) 계속(잔여 스코프: accuracy(부분)/lotto-lib) — gap trigger 4종 전부 미근접.
...
apps/moneyball/src/lib/teams/ 스코프 exported type/interface 18개 grep → 0-external-hit 5개 후보. general-purpose subagent 독립 재검증 — 4/5 CONFIRMED_UNEXPORTED, 1건 FALSE_POSITIVE(TeamRecentGame — cross-directory wave-* 테스트 4개 named import, 정확히 걸러냄). 3개 파일 export 키워드만 제거: TeamEloTrendData / TeamPitcherRow / HomeAwayGame / TeamUpcomingGame.

`pnpm --filter moneyball type-check` clean, test 581/581파일 4528/4528 green. commit b89c1865, R4 직push(단일 논리 단위, PR 생략).

다음 사이클 추천 = review-code(heavy) 계속(잔여 스코프: dashboard 16, matchup 14, debug 14) — gap trigger 4종 전부 미근접.
...
apps/moneyball/src/app/mlb + lib/mlb 스코프 exported type/interface 41개 grep(cycle 2850 cwd 버그 재발 방지 — apps/packages/scripts 명시 스코프 + exclude-dir node_modules/.next/.turbo) → 0-external-hit 30개 후보. general-purpose subagent 독립 재검증(재grep + barrel re-export 확인 + 테스트 컨슈머 확인) — 30/30 CONFIRMED_UNEXPORTED, false positive 0건. 14개 파일 export 키워드만 제거(타입 유지).

`pnpm --filter moneyball type-check` clean, test 581/581파일 4528/4528 green. commit 993bf814, R4 직push(PR 생략).

부산물: `apps/moneyball/src/app/mlb/reviews/reviews-data.ts:59` `MlbReviewsData` export 제거 후 file-local 도 미사용(lint warning) — 다음 review-code(heavy) 사이클 죽은 코드 제거 후보.

다음 사이클 추천 = review-code(heavy) 계속(잔여 스코프: reviews 27, teams 18, dashboard 16, matchup 14, debug 14 — mlb 완료) — gap trigger 4종 전부 미근접.
...
`scripts/op-analysis-ce-cohort.ts` 재실행 — n=367(CE 320/비CE 47), cycle 2821과 완전 동일 수치(20-cycle 경과에도 신규 verified 예측 0건, DB 파이프라인 정상·cycle 처리 속도 ≫ 실제 경기일 간격 패턴 지속). 격차 8.8pp 유지, overlap 월(05/06/07) 통제 격차 10.8pp ≈ 유지, LLM 부가가치 우세 결론 21회 연속 재확인. CREDIT_EXHAUSTED·비CE 표본 동결(마지막 예측 2026-07-01, 64일+ 경과) 상태 변화 없음.

코드 변경 없음(CLAUDE.md 데이터 갱신만, 커밋 753fefaf) — 별도 code PR/ship 없음.

skill-evolution trigger 평가: cycle_n % 50 = 41(미충족), 직전20(2822-2841) chain pool 분포 다양(review-code(heavy)/polish-ui/fix-incident/lotto/op-analysis/info-arch 혼재, trigger5 미충족), meta-pattern/chain-evolution 미발화(trigger1/4 미충족), 5연속 fail 없음(trigger2 미충족). emergency stop 미충족(직전10 중 success 다수).

다음 사이클 추천 = review-code(heavy) 계속 또는 gap-fill 자연 대기(lotto 30/30 도달 예상, fix-incident 15/20).
...
commit-count tier 5-7건 API 라우트 중 미감사 3개(`picks/submit/route.ts`/`leaderboard/sync/route.ts`/`kbo-scores/route.ts`) 발견 → general-purpose 서브에이전트 5축 감사(select 컬럼 소비/comment-code 정합/dead code/CRON_SECRET 보호/caller shape 정합). 전부 clean — 3개 모두 client-invoked 공개 엔드포인트(cron 미대상)라 CRON_SECRET 부재 정상(origin-check 사용). `kbo-scores/route.ts` 의 `NaverGame.homeTeamEmblemUrl`/`awayTeamEmblemUrl` interface 필드 미소비 발견했으나 TS interface(컴파일타임 erased, runtime cost 0)라 비-버그 판단, 수정 안 함.
...
side-finding: `gh run list` + `vercel ls --prod` 확인 결과 이번 사이클 시작 시점 push(5ee753fb)에 GitHub→Vercel webhook 이 자동으로 새 git-source 배포를 생성 — cycle 2807 수동 CLI 배포와 별개. cycle 2807 이 남긴 "다음 실제 push 후 webhook 재개 확인" 질문에 자연 답변 — webhook 자체 회복(과거 self-healed 패턴과 동일), 신규 조치 불필요.

review-code(heavy) 백로그가 cycle 2805/2806 에서 두 번 소진 선언됐으나, git log 커밋 수 기준 components/+lib/ 디렉토리 sweep 으로 미감사 신규 후보군(1-commit 파일) 발견. MLB matchup 컴포넌트 3개(MlbMatchupConvergencePickRecord/MlbMatchupRecentForm/MlbMatchupSeasonHeadToHead) KBO 대응 원본과 parity 감사 + buildDailyAccuracy.ts 단독 감사.
...
- carry-over(cycle 2720) 미감사 잔여 3개 중 `buildSeasonSummary.ts`(365줄) 전수감사 — `SeasonSummary` 전 필드 `seasons/[year]/page.tsx` 소비 교차검증, computed-but-unconsumed/stale-comment 계열 0건. 한국시리즈 동점 tie-break는 기존 fix 반영 상태 확인.
- 코드 변경 없음.
- 다음 사이클 추천 = review-code(heavy) rotation 계속(`compareModels.ts`/`hub-dispatch.ts` 잔여 2개) 또는 info-architecture-review(gap 13/30).

## cycle 2720 (2026-09-01) — SUCCESS — review-code(heavy)

- 진단: open issue 0, unprocessed plan 0/23(approved 없음). fix-incident gap 도달(직전20 0회)했으나 `gh run list`(10/10 success/skipped) + 최근 debug fix 커밋 0건 → 무이슈 재확인(4연속). lotto gap 30/30 도달했으나 `~/lotto_picks/2026-09-05-50sets.md`(다음 회차 picks) + `2026-08-29-result.md`(직전 회차 OOS) 둘 다 오늘 cron self-heal로 이미 생성 완료 — 실질 갭 없음(cycle 2718과 동일 패턴). 직전8 distinct=3 — 2-chain lock 미충족.
- carry-over(cycle 2719) 미감사 잔여 4개 중 최대 `buildPicksStats.ts`(393줄) 서브에이전트 위임 전수감사 — 3건 확인:
  - `PickEntry.status`: route.ts에서 계산만 되고 어떤 UI도 안 읽음(각 컴포넌트가 isResolved+myIsCorrect로 자체 배지 계산) — 필드+계산 제거.
  - `ai_confidence`: PickGameResult 파이프라인 전체(KBO `predictions.confidence` select column, MLB `deriveMlbOutcome` confidence 구조분해, PickEntry 전달)에서 repo 어디도 안 읽음 — select 컬럼부터 타입까지 전체 제거.
  - `FactorAgreement.byFactor` 독스트링이 "ACTIVE_FACTOR_KEYS 안 factor key로 제한"이라 서술했지만 실제 로직은 정반대(entries 안 실제 등장한 key 전부 수집, 운영 가중치 변경이 과거 분석에 영향 없도록 하는 의도된 설계) — 정정.
- fix: 4개 파일 수정(route.ts/buildPicksStats.ts/테스트 2개). tsc/eslint clean, 전체 테스트 571파일 4483건 green. PR #3073 squash merge(10210387) — state=MERGED 실측 확인.
...
- review-code(heavy) rotation 계속 — cycle 2712/2713/2714 carry 잔여 중 `TodayGameCard` badge 8필드 오도 이슈 채택.
- `homeRank`/`awayRank`/`homeRecent10`/`awayRecent10`/`h2hHomeWins`/`h2hAwayWins`/`homeTeamVenue`/`awayTeamVenue` 8필드가 `TodayGameCard` 인터페이스에 optional 선언돼있지만 `getTodayAnalysisData()`는 전혀 계산 안 함(항상 undefined) — 유일 caller `analysis/page.tsx`의 `gamesWithRank`가 rankMap/venueMap/recent10Map/h2hMap 기반으로 즉시 재계산·override해 기능 손실은 없었으나 인터페이스가 함수 책임을 오도.
...
- 서브에이전트 위임 감사 — 전체 export interface/함수 소비처 교차검증. `BestPickCard.confidence`(getBestPickOfWeek/getUpsetPickOfMonth), `MlbUpcomingGame.homeWinProb`/`duelValidCount`(getMlbThisWeekRemainingGames) 모두 계산·리턴되지만 유일 caller(analysis/page.tsx, mlb+en/mlb analysis page.tsx weekRemaining 블록)가 미소비 확인.
- fix: 3개 필드 인터페이스+계산 로직에서 제거(commit f8d38221). tsc/eslint clean, 관련 테스트 28파일 181건 green.
- 부가 발견(변경 보류) — `TodayGameCard`의 badge 관련 8개 필드는 `analysis-data.ts`가 실제 계산하지 않고 page.tsx가 즉시 덮어씀(기능 무해, 인터페이스만 오도) / `prediction_type` select 컬럼이 filter 용도로만 선택되고 응답 후 미사용(대역폭만 소모, 저우선). 둘 다 이번 사이클 범위 밖 — 다음 후보로 carry.
- gap trigger 4종 전부 미도달(fix-incident 16/20, op-analysis 21/25, info-arch 3/30, lotto 23/30). 직전8 distinct=3 — 2-chain lock 미충족.
