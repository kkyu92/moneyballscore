...
function DailyTrend({
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      <h3 className="text-sm text-gray-500 dark:text-gray-400 mb-4">일자별 추세 (7일, KST)</h3>
      ...
      <div className="flex gap-3 text-xs text-gray-500 dark:text-gray-400 mt-3">
...
function CategoryList({
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      <h3 className="text-sm text-gray-500 dark:text-gray-400 mb-3">{title}</h3>
      {data.length === 0 ? (
        <p className="text-gray-400 dark:text-gray-500 italic text-sm">데이터 없음</p>
      ) : (
      ...
      {stats.error ? (
        ...
        <>
          ...
          <section className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <h3 className="text-sm text-gray-500 dark:text-gray-400 mb-2">최근 7일 이벤트</h3>
              <p className="text-4xl font-bold">{stats.total}</p>
              <div className="text-xs text-gray-500 dark:text-gray-400 mt-2 flex flex-wrap gap-x-3 gap-y-1">
                <span>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {stats.samples.map((log) => (
                      <tr key={log.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-3 py-2 font-mono text-xs text-gray-600 dark:text-gray-400 truncate max-w-[140px]">
                          {log.backend}
                        </td>
                        <td className="px-3 py-2 font-mono text-xs">
                          {log.violation_type}
