...
describe.each([
  ...
  it('팀별 수렴 픽 성적 섹션 존재함', () => {
    ...
    expect(src).toContain('팀별 수렴 픽 성적');
  });

  it('강수렴 배지 라벨 존재함', () => {
    expect(src).toContain('🏅 강수렴:');
  });

  it('완전수렴 배지 라벨 + amber 테마 존재함', () => {
    expect(src).toContain('★ 완전수렴:');
  });

  it('null/empty 시 섹션 숨김 가드 존재함', () => {
    expect(src).toContain('strongTeamStats.length > 0 || completeTeamStats.length > 0');
  });

  it('UPCOMING_CONVERGENCE_TEAM_LIMIT 로 표시 개수 제한됨', () => {
    expect(src).toContain('strongTeamStats.slice(0, UPCOMING_CONVERGENCE_TEAM_LIMIT)');
    expect(src).toContain('completeTeamStats.slice(0, UPCOMING_CONVERGENCE_TEAM_LIMIT)');
  });
});
