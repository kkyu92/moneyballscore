...
describe('silent drift wave 193 — CF Worker cron fire count + role count stale → 실제 값 정합 (cycle 1459)', () => {
  it('wrangler.toml: 총 fire/day 합계 82 → 87 갱신 (MLB 6회 포함, 폐기된 self-develop +1 제거, plan #25 Phase 2 elo_update +1 — cycle 2082)', () => {
    const src = readFileSync(join(REPO_ROOT, 'cloudflare-worker/wrangler.toml'), 'utf8');
    expect(src).not.toMatch(/15 \+ 24 \+ 42 \+ 1 = 82/);
    expect(src).toMatch(/15 \+ 24 \+ 42 \+ 6 = 87/);
  });

  it('wrangler.toml: MLB pipeline 회수 4회/일 → 6회/일 (UTC 18-22 5회 + UTC 10 1회, plan #25 Phase 2 elo_update 추가 — cycle 2082)', () => {
    const src = readFileSync(join(REPO_ROOT, 'cloudflare-worker/wrangler.toml'), 'utf8');
    expect(src).not.toMatch(/MLB pipeline.*4회\/일/);
    expect(src).not.toMatch(/MLB pipeline.*5회\/일/);
    expect(src).toMatch(/MLB pipeline.*6회\/일/);
  });
  ...
  it('worker.ts: JSDoc 7번 항목 = MLB pipeline 6회/일 명시 (plan #25 Phase 2 elo_update 추가 — cycle 2082)', () => {
    const src = readFileSync(join(REPO_ROOT, 'cloudflare-worker/src/worker.ts'), 'utf8');
    expect(src).toMatch(/7\) "17 18-22,10 \* \* \*" — MLB pipeline/);
    expect(src).toMatch(/6회\/일/);
  });
});
