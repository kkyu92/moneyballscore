import { ImageResponse } from "next/og";
import { MLB_TEAMS, type MlbTeamCode } from "@moneyball/shared";
import { MLB_FACTOR_COUNTS } from "@moneyball/kbo-data";

export const runtime = "nodejs";
export const size = { width: 1200, height: 630 };
export const contentType = "image/png";
export const alt = "MLB Team Profile - MoneyBall Score";

interface Props {
  params: Promise<{ code: string }>;
}

function isMlbTeamCode(v: string): v is MlbTeamCode {
  return v in MLB_TEAMS;
}

export default async function Image({ params }: Props) {
  const { code } = await params;
  const team = isMlbTeamCode(code) ? MLB_TEAMS[code] : null;
  const teamName = team?.name ?? code;
  const league = team ? `${team.league} ${team.division}` : "MLB";
  return new ImageResponse(
    (
      <div
        style={{
          width: "100%",
          height: "100%",
          display: "flex",
          flexDirection: "column",
          background:
            "linear-gradient(135deg, #0d1f2b 0%, #1f3b5c 50%, #3f7ba0 100%)",
          color: "white",
          padding: "72px 80px",
          fontFamily: "sans-serif",
        }}
      >
        <div
          style={{
            display: "flex",
            alignItems: "center",
            gap: 18,
            fontSize: 36,
            opacity: 0.92,
            letterSpacing: "-0.5px",
            fontWeight: 600,
          }}
        >
          <span style={{ fontSize: 48,
          display: "flex",
        }}>⚾</span>
          <span>MoneyBall Score</span>
        </div>

        <div
          style={{
            marginTop: 72,
            fontSize: 104,
            fontWeight: 800,
            letterSpacing: "-3px",
            lineHeight: 1,
            display: "flex",
          }}
        >
          {teamName}
        </div>

        <div
          style={{
            marginTop: 24,
            fontSize: 40,
            opacity: 0.88,
            letterSpacing: "-1px",
            display: "flex",
          }}
        >
          {league} · MLB 팀 프로필
        </div>

        <div
          style={{
            marginTop: 32,
            display: "flex",
            flexWrap: "wrap",
            gap: 12,
          }}
        >
          {["FIP", "wOBA", "xwOBA", "Park Factor", "예측 적중률"].map((tag) => (
            <div
              key={tag}
              style={{
                padding: "10px 20px",
                background: "rgba(255,255,255,0.14)",
                borderRadius: 999,
                fontSize: 26,
                fontWeight: 600,
          display: "flex",
        }}
            >
              {tag}
            </div>
          ))}
        </div>

        <div
          style={{
            marginTop: "auto",
            display: "flex",
            alignItems: "center",
            justifyContent: "space-between",
            fontSize: 26,
            opacity: 0.7,
          }}
        >
          <span>moneyballscore.vercel.app/mlb/team/{code}</span>
          <span>Team Profile · {MLB_FACTOR_COUNTS.total} Factors</span>
        </div>
      </div>
    ),
    { ...size },
  );
}
