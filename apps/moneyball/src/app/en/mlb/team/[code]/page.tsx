...
// KO parity (fmtGamesBehind, page.tsx cycle 2217): rank badge already shows position,
// so this only renders the numeric GB gap.
function fmtGamesBehind(gb: number | null): string {
  if (gb === null) return "";
  return ` · GB ${gb % 1 === 0 ? `${gb}.0` : gb}`;
}
...
export default async function MlbTeamPageEn({ params }: PageProps) {
  ...
  const upcoming = await buildMlbTeamUpcoming(code).catch((err) =>
    captureFallback(err, [], { route: "/en/mlb/team/[code]", source: "buildMlbTeamUpcoming" }),
  );
  ...
  const [strongTeamStats, completeTeamStats, divisionStandings] = await Promise.all([
    ...
    buildMlbDivisionStandings().catch((err) =>
      captureFallback(err, null, { route: "/en/mlb/team/[code]", source: "buildMlbDivisionStandings" }),
    ),
  ]);
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <span className="text-xs px-2 py-1 rounded bg-gray-100 dark:bg-[var(--color-surface-card)] text-gray-600 dark:text-gray-300 font-mono">
            ...
            {divisionRank && (
              <>
                {" "}
                {divisionRank.rank}/{divisionRank.total}
      ...
      {eloTrend.points.length > 0 && (
        ...
        >
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">
            Season Elo change by game · dotted line = league average
          </p>
          <MlbTeamEloChart points={eloTrend.points} teamCode={code} locale="en" />
        </section>
      )}
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200 dark:border-[var(--color-border)] text-left text-xs text-gray-500 dark:text-gray-400">
                  ...
                  <th className="py-2 font-medium text-right">Model Pick</th>
                </tr>
              </thead>
              <tbody>
                {upcoming.map((u) => {
                  ...
                  const detailHref = `/en/mlb/games/${u.gameDate}/${slugA}-vs-${slugB}`;
                  return (
                    <tr
                      key={u.gameId}
                    ...
                    >
                      ...
                      <td className="py-2 pr-3">
                        {u.opponentCode ? (
                          <Link href={`/en/mlb/team/${u.opponentCode}`} className="hover:text-brand-500">
                      ...
                      <td className="py-2 text-right text-xs text-gray-700 dark:text-gray-200">
                        {u.predictedAsWinner ? "This team" : "Opponent"}
                        {probPct != null && (
