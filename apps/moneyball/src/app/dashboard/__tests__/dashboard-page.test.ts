...
describe("dashboard DASHBOARD_ISR_SECONDS source-of-truth guard (silent drift wave 131 cycle 1350)", () => {
  it("dashboard/page.tsx revalidate = 86400 magic 부재 — DASHBOARD_ISR_SECONDS 사용", () => {
    expect(PAGE_SRC).not.toMatch(/export\s+const\s+revalidate\s*=\s*86400\b/);
    expect(PAGE_SRC).toMatch(/export\s+const\s+revalidate\s*=\s*DASHBOARD_ISR_SECONDS/);
  });
  ...
  it("v2-preview/page.tsx revalidate = 86400 magic 부재 — DASHBOARD_ISR_SECONDS 사용", () => {
    expect(V2_PREVIEW_SRC).not.toMatch(/export\s+const\s+revalidate\s*=\s*86400\b/);
    expect(V2_PREVIEW_SRC).toMatch(/export\s+const\s+revalidate\s*=\s*DASHBOARD_ISR_SECONDS/);
  });
