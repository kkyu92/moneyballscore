...
export function ConvergenceTeamStatsBadges<T extends string = string>({
  ...
  const t = {
    title: locale === 'en' ? 'Convergence Pick Record by Team' : '팀별 수렴 픽 성적',
    strong: locale === 'en' ? '🏅 Strong:' : '🏅 강수렴:',
    complete: locale === 'en' ? '★ Full:' : '★ 완전수렴:',
  };
          ...
          {strongTeamStats.slice(0, UPCOMING_CONVERGENCE_TEAM_LIMIT).map((stat) => {
            ...
            return (
              <span
                ...
                title={
                  locale === 'en'
                    ? `${nameResolver(stat.teamCode)}: ${stat.wins}W ${stat.losses}L (${pct}%) — ${teamTotal} strong-convergence picks`
                    : `${nameResolver(stat.teamCode)}: ${stat.wins}승 ${stat.losses}패 (${pct}%) — 강수렴 픽 ${teamTotal}경기`
                }
              >
          ...
          {completeTeamStats.slice(0, UPCOMING_CONVERGENCE_TEAM_LIMIT).map((stat) => {
            ...
            return (
              <span
                ...
                title={
                  locale === 'en'
                    ? `${nameResolver(stat.teamCode)}: ${stat.wins}W ${stat.losses}L (${pct}%) — ${teamTotal} full-convergence picks`
                    : `${nameResolver(stat.teamCode)}: ${stat.wins}승 ${stat.losses}패 (${pct}%) — 완전수렴 픽 ${teamTotal}경기`
                }
              >
