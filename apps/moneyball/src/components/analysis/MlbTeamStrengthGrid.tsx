...
function StreakTag({ streak }: { streak: MlbTeamStrengthRow['streak'] }) {
...
function RecentRecordBar({ record }: { record: MlbTeamStrengthRow['recentRecord'] }) {
 ...
 */
export function MlbTeamStrengthGrid({ rows }: Props) {
    ...
    <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
      {rows.map((row, idx) => (
        ...
        >
          ...
          <RecentRecordBar record={row.recentRecord} />
        </Link>
      ))}
    </div>
  );
