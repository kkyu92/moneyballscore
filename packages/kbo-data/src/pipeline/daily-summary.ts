...
//
// 기존 `const { data } = await db.from('predictions').select(...)` 는
...
// silent skip = 사용자 무감지 ghost notification. CLAUDE.md 사례 3 (predictions
// `.error` 미체크 silent fail) 패턴 동일 영역 (daily.ts) 재현 차단.
//
