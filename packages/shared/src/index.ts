...
/**
 * 매치업 페이지 최근 폼 표시 경기 수 — /matchup/[teamA]/[teamB] recent form section.
 * RECENT_FORM_GAMES(10, 예측 모델용) 와 별개 — display compact 5경기.
 * buildTeamRecentForm default + matchup page callsite 단일 source.
 * silent drift family wave-362 (cycle 1700).
 */
export const MATCHUP_RECENT_FORM_GAMES = 5;
 ...
 * buildGameOverview "저득점 예상" 태그 임계 — wave-340 (cycle 1675).
 ...
 * · wave-473 팩터 N:M 균형 표시 확장 (cycle 1835): factorFavoredCount != null(homeWins/awayWins non-null, favored-first 포맷) — isPickGame colored·bold, 비수렴 gray. wave-415 isPickGame 조건 → factorFavoredCount != null 확장. 변경 시 analysis/page.tsx wave-415·473 factorHasData callsite 동시 조정.
...
/**
 * 팩터 수렴 픽 완전 수렴 임계 — wave-408 (cycle 1755). · wave-415 배지 색상 (cycle 1766). · wave-452 game/[id] amber 배지 (cycle 1812). · wave-454 chip amber 색상 (cycle 1814). · wave-456 !isComplete opponent chip 조건 (cycle 1816). · wave-459 analysis/page.tsx isComplete chip 색상 (cycle 1819). · wave-461 game/[id]/page.tsx 합치 칩(✓ 합치) amber 3-tier 색상 (cycle 1822). · wave-463 game/[id]/page.tsx '완전수렴' 레이블 칩 (cycle 1823). · wave-465 analysis/page.tsx '완전수렴' 레이블 칩 (cycle 1825). · wave-467 analysis/page.tsx sectionHasComplete — 섹션 border/bg amber upgrade (cycle 1828). · wave-475 예정 경기 팩터 N:M 색상 amber 티어 (cycle 1837): |convergenceNetScore| ≥ 본 값 시 text-[var(--color-accent)] 골드 색상.
 * |compositeDuelScore| ≥ 본 값 시: analysis/page.tsx = accent(골드) 색상·amber 칩·'완전수렴' 레이블 칩 (wave-459/wave-465) + 섹션 container amber border/bg + 제목/성적 텍스트 amber (wave-467), game/[id]/page.tsx = amber 배경 배지 + amber 칩 + 합치 칩 amber + '완전수렴' 레이블 칩 (wave-461/wave-463). false 시 상대 팀 우세 팩터 칩 표시 (wave-456).
...
/**
 * wave-557: 강수렴 픽 팀별 시즌 성적 최소 경기 수 임계.
 * 팀이 강수렴 픽으로 지목된 경기가 본 값 미만이면 표시 생략 (소표본 노이즈 차단).
 * 변경 시 convergenceRecord.ts getConvergencePickTeamStats + analysis/page.tsx wave-557 callsite 동시 조정.
 */
 ...
 * 변경 시 convergenceRecord.ts computeConvergenceStreak + computeConvergenceBestStreak callsite 동시 조정.
 */
...
/**
 * wave-452: 게임 상세 페이지 팩터 수렴 픽 배지 가중 우위 강조 임계값 (%). · wave-454 chip brand 색상 (cycle 1814). · wave-459 analysis/page.tsx isWeightStrong chip 색상 (cycle 1819).
