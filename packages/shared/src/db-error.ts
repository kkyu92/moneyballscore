 ...
 *
 * 기존 `const { data } = await db.from(...)` 패턴이 DB 오류 시 data=null silent
 * fallback → silent drift 누적 (existingSet 빈 → 중복 insert 큐, count=null →
 * false positive GAP, summary skip 위장).
 *
 * 본 helper 는 모든 supabase select / count / write 응답을 단일 검증 채널로
 * 통과시켜 `.error` 미체크 silent drift 를 fail-loud 로 전환.
 *
 * apps/moneyball 과 packages/kbo-data 양쪽에서 단일 helper 통과 (cross-package 일관성).
 */
