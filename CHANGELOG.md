...
- cycle 2892 checkpoint 커밋(a66064b8) 이후 88 commits 전수 대조: 신규 라우트 0건, breadcrumb 누락 18건 불변(전부 의도된 noindex/debug/redirect-only), MegaMenu/Footer/Header/sitemap.ts 변경 0건.
- "현 IA 충분" 9연속 재확정(2679→2709→2739→2769→2800→2830→2860→2892→2922). 코드 변경 없음(checkpoint 문서만). `docs/design/ia-2026-09-18-cycle-2922-30-cycle-gap-checkpoint.md` 박제.
- 다음 재도달 목표 = cycle 2952 근방.
...
- 진단: open issue 0, unprocessed approved plan 0/23(전부 completed/archived/spec-only-deferred). 2차 방어선(cycle 2914 retro commit 03ffcd3d) OK. 직전8(2907-2914) distinct=4(review-code6+polish-ui1+explore-idea1+lotto1) — 2-chain lock 미충족. gap trigger 4종(fix-incident 17/20·op-analysis 16/25·info-arch 23/30·lotto 1/30 방금 발화) + explore-idea saturation(11/15) 전부 미근접/미충족. 1242회 로또 추첨(9/19 21시) 미도달. cycle 2914 추천대로 components/ 2차 스윕 계속, matchup(1399)+picks(1856) 배치(총 3255줄, 23 exports) 선정.
- general-purpose subagent 독립 검증(repo 전체 실제 import grep) — 23개 exported symbol 전부 CONFIRMED_UNUSED 0건, 전부 외부 실사용 확인(matchup 10건 app/matchup·app/mlb/matchup 계열, picks 13건 app/picks·app/page·MyPicksClient 계열). de-export 대상 없음 — 코드 변경 0.
- 잔여 스윕 대상 = analysis(1421)/dashboard(1936)/layout(1820)/predictions(3580)/shared(1395) 5개 디렉토리(~10150줄). 다음 사이클 추천 = components/ 2차 스윕 계속(위 5개 중 배치) 또는 info-architecture-review(gap 24/30) 또는 1242회 로또 추첨(9/19 21시) 이후 lotto OOS.
...
- `apps/moneyball/src/lib/teams/` 스코프 exported type/interface 21개 grep → 20개는 cross-file import(page.tsx/컴포넌트/buildMlbTeamProfile.ts 재사용 등) 확인되어 유지. `TeamProfile`(buildTeamProfile.ts:25)만 자기 파일 내부(함수 리턴 타입 추론)로만 사용 — 외부 어디서도 타입 자체를 이름으로 import 안 함(buildTeamProfile 함수 호출부는 추론만 의존). general-purpose subagent 독립 재검증(전체 repo 재grep + barrel/index.ts 부재 확인 + 테스트 파일 확인 + name-collision(MlbTeamProfile) 배제) — 1/1 CONFIRMED_UNUSED, false positive 0건. export 키워드만 제거.
...
- apps/moneyball/src/lib/reviews/ 스코프 exported type/interface 13개 grep → MonthRange/WeekRange/MlbWeekly*/Weekly*/PredictionRow/MlbPredictionRow 등 11개는 build*Review.ts 계열 cross-file import(page.tsx 재-export 경유 포함) 확인되어 유지. ReviewRangeLocale(computeMonthRange.ts + computeWeekRange.ts, 각각 독립 정의·서로 미참조)만 자기 파일 내부 함수 파라미터 기본값으로만 사용 확인. general-purpose subagent 독립 재검증(전체 repo 재grep + barrel/index.ts 부재 확인 + 테스트 파일 확인 + 타 패키지 name-collision 확인) — 2/2 CONFIRMED_UNUSED, false positive 0건. export 키워드만 제거.
...
- apps/moneyball/src/lib/predictions/ + lib/v2-shadow-monitor/ 스코프 exported type/interface 6개 grep → V2PreviewResult(app/v2-preview/page.tsx cross-file import) + CohortDoc(loader.ts cross-file import) 2개만 유지, 나머지 4개 0-external-hit 후보. general-purpose subagent 독립 재검증(전체 repo 재grep + barrel/index.ts 부재 확인 + 테스트 파일 type import 여부) — 4/4 CONFIRMED_UNUSED, false positive 0건. export 키워드만 제거: V2WeightKey / V2PreviewDelta (v2Predictor.ts), CohortTable (parse.ts), CohortFile (loader.ts).
...
- apps/moneyball/src/lib/leaderboard/ 스코프 exported type/interface 7개 grep → LeaderboardEntry/LeaderboardSyncPayload/MlbLeaderboardSyncPayload/LeaderboardMode/AiBaseline 5개는 cross-file import(컴포넌트/API route/서버 함수) 확인되어 유지, SyncState/LeaderboardState 2개는 use-leaderboard.ts 내부 전용 사용만 확인. general-purpose subagent 독립 재검증(전체 repo 재grep + barrel/index.ts 부재 확인 + 테스트 파일 type import 여부 + `ReturnType<typeof useLeaderboard>` 사용 여부 + packages/ 참조 확인) — 2/2 CONFIRMED_UNUSED, false positive 0건. export 키워드만 제거: SyncState / LeaderboardState (use-leaderboard.ts).
...
### review-code(heavy): export-but-unused heuristic analysis 스코프 6건 (cycle 2865, SUCCESS)

- 진단: open issue 0, unprocessed approved plan 0/23(전부 archived/completed/pending-user, `status: approved` 0건). 2차 방어선(cycle 2864 retro commit f2d111e6) OK. 직전8(2857-2864) distinct=3(review-code(heavy)6+info-architecture-review1+polish-ui1) — 2-chain lock 미발동. **fix-incident gap trigger 정확 20/20 도달(마지막 발화 cycle 2845)** — lite 점검 실행: `pipeline_runs` 최근 7일 187건 전부 status=success(에러 0%), `gh run list` CI/Vercel Deploy 워크플로 전부 success 또는 skipped(실패 0건), open issue 0건 — 실제 incident 부재 확인 후 정상 종료. op-analysis(13/25)/info-arch(5/30)/lotto(23/30) 전부 미근접. ship-0 emergency stop 미충족(직전10 9 success+1 retro-only). cycle 2864 next_recommended("review-code(heavy) 계속 — analysis 우선") 채택.
- apps/moneyball/src/lib/analysis/ 스코프 exported type/interface 8개 grep → FactorRawDetails(app/analysis/game/[id]/page.tsx + GameAnalysisProse.tsx + DetailedFactorAnalysis.tsx cross-file import) + GameOverview(app/analysis/game/[id]/page.tsx + GameOverview.tsx + packages/kbo-data/src/factors/mlb-overview.ts cross-file import) 2개만 유지, 나머지 6개 0-external-hit 후보. general-purpose subagent 독립 재검증(전체 repo 재grep + barrel/index.ts 부재 확인 + co-located 테스트 cross-file import 구분 — 3개 테스트 파일 모두 함수만 import, 타입 심볼 import 0건 확인) — 6/6 CONFIRMED_UNUSED, false positive 0건. export 키워드만 제거: MlbCompositeDuelInput / MlbCompositeDuelResult (computeMlbCompositeDuel.ts), CompositeDuelInput / CompositeDuelResult (computeCompositeDuel.ts), FactorExplanation / GameOverviewInput (factor-explanations.ts).
- `pnpm --filter moneyball exec tsc --noEmit` clean, test 581/581파일 4560/4560 green, lint 0 errors(기존 warning 1건 유지). commit c0b0cd1e, R4 직push(단일 논리 단위, PR 생략).
...
- apps/moneyball/src/lib/standings/ 스코프 exported type/interface 8개 grep → EloDataPoint(EloTrendChart.tsx 컴포넌트 prop 실사용) + TeamAccuracyRow(teams/page.tsx cross-file import) 2개만 유지, 나머지 6개 0-external-hit 후보(StandingRow 는 buildStandings.ts 의 dead 재-export — 실제 소비자는 @moneyball/kbo-data 원본에서 직접 import). general-purpose subagent 독립 재검증(전체 repo 재grep + barrel/index.ts 부재 확인 + co-located 테스트 cross-file import 구분 + packages/ 동명 무관 심볼 확인) — 6/6 CONFIRMED_UNUSED, false positive 0건. export 키워드만 제거(또는 재-export 라인 삭제): StandingRow (buildStandings.ts), EloTrendData (buildEloTrend.ts), MagicNumberTeam (computeMagicNumber.ts), MatchupRow / TeamHomeAwayRow / TeamBiasRow (buildTeamAccuracy.ts).
- `pnpm --filter moneyball exec tsc --noEmit` clean, test 581/581파일 4560/4560 green, lint 0 errors(기존 warning 1건 유지). commit b7278e14, R4 직push(단일 논리 단위, PR 생략).
...
- apps/moneyball/src/lib/insights/ 스코프 exported type/interface 8개 grep → DebateTimelineData(app/analysis/game/[id]/page.tsx:654 실사용 확인)만 유지, 나머지 7개 0-external-hit 후보. general-purpose subagent 독립 재검증(전체 repo 재grep + barrel/index.ts 부재 확인 + co-located vs cross-directory 테스트 import 구분 + `StatusBadge` 동명 무관 로컬 함수(LiveScoreboard.tsx) 오탐 배제 + `InsightEntry` 테스트 문자열 검사(readFileSync/regex, 실제 import 아님) 오탐 배제) — 7/7 CONFIRMED_UNEXPORTED, false positive 0건. export 키워드만 제거: StatusBadge (statusBadge.ts), DebateArgumentSummary / DebateCalibrationSummary / InsightEntry (loader.ts), SeriesTopic / SeriesEntry (series.ts), TopFactor (topFactors.ts).
...
- apps/moneyball/src/lib/matchup/ 스코프(buildMatchupEloTrend/buildMatchupProfile/buildMatchupUpcoming/buildSeasonHeadToHead/canonicalPair) exported type/interface 14개 grep → 0-external-hit 10개 후보. general-purpose subagent 독립 재검증(전체 repo 재grep + barrel/index.ts 부재 확인 + components/matchup·app/matchup 컨슈머 확인 + wave-* 테스트 확인 + packages/shared 동명 심볼 별개 확인) — 10/10 CONFIRMED_UNEXPORTED, false positive 0건. export 키워드만 제거: MatchupEloTrendData (buildMatchupEloTrend.ts), MatchupSideStat/MatchupProfile/MatchupStreak/MatchupRecentRecord/MatchupAvgMargin/MatchupBlowoutStats/MatchupCloseGameStats/MatchupHomeAwaySplit (buildMatchupProfile.ts), MatchupUpcomingGame (buildMatchupUpcoming.ts).
...
### review-code(heavy): export-but-unused heuristic dashboard 스코프 7건 (cycle 2856, SUCCESS)
...
- apps/moneyball/src/{app,components,lib}/dashboard/ 스코프 exported type/interface 19개 grep → 0-external-hit 7개 후보. general-purpose subagent 독립 재검증(전체 repo 재grep + barrel/index.ts 부재 확인 + 테스트 컨슈머 확인 + 동명 심볼 충돌 확인) — 7/7 CONFIRMED_UNEXPORTED, false positive 0건. export 키워드만 제거: ConfidenceBucketInput / ConfidenceBucket (buildConfidenceBuckets.ts), ModelGroupStats / DailyStat (compareModels.ts), DailyAccuracyInput (buildDailyAccuracy.ts), CategoryBreakdown (buildHallucinationStats.ts), FactorStat (factor-accuracy.ts).
...
- 진단: open issue 0, unprocessed approved plan 0/23. 2차 방어선(cycle 2831 retro commit 07effa6b) OK. 직전8 distinct=4(review-code(heavy)/review-code/fix-incident/info-architecture-review) — 2-chain lock 미충족. gap trigger 4종 전부 미도달(fix-incident 5/20, op-analysis 11/25, info-arch 2/30, lotto 20/30). dominance-positive streak 자연 지속.
- 방법론: git 1-commit heuristic 후보를 TODOS.md/CHANGELOG.md 텍스트 교차검증 — 매칭되는 후보 전부 이미 과거 사이클에서 CLEAN 확정된 파일들(stale candidate)임을 확인, 대신 TODOS/CHANGELOG 어디에도 이름이 전혀 등장하지 않는(0 hits) 진짜 미감사 파일 7개(`accuracy/shadow-pair-prob.ts`/`dashboard/buildHallucinationStats.ts`/`predictions/v2Predictor.ts`/`seo/json-ld.ts`/`tabpfn-import.ts`/`tabpfn-export.ts`/`debug/factorDeltaStats.ts`/`debug/silentDriftStats.ts`) 8개 확보.
- general-purpose 서브에이전트 전수 감사: 4/8 CLEAN, 4건 발견 — (1) **`debug/silentDriftStats.ts`**: `daily.ts` verify mode 가 `pipeline_runs.predictions` 를 모든 종료 경로에서 하드코딩 0 으로 insert(실제 검증 건수는 인메모리 `verifiedCount.value` 에만 존재, 영속화 안 됨) → 본 helper 가 이 컬럼을 `games_found>0 && predictions===0` silent-drop proxy 로 재사용하는 `/debug/silent-drift` 대시보드가 **정상적인 verify run 도 전부 silent drift 로 오판정**(자기 목적을 스스로 무력화하는 아이러니한 버그). 실시간 Sentry/Telegram 경보(`captureSilentDriftAlert`)는 별도로 `verifiedCount` 를 직접 받아 판단하므로 영향 없음. (2) `predictions/tierStats.ts` 계열과 별개로 `tabpfn-import.ts` 헤더 주석이 "/accuracy/shadow page 에서 tabpfn-shadow 조회됨" 이라 명시하지만 실제 그 페이지 쿼리는 `scoring_rule in [CURRENT_SCORING_RULE, SHADOW_SCORING_RULE]` 만 포함해 tabpfn-shadow 는 제외, `pairProbForRow()` 도 해당 분기 부재 — write-only 상태(stale comment). (3) `seo/json-ld.ts` 헤더가 "insights/page.tsx·analysis/game/[id]/page.tsx·guide/page.tsx 의 inline JSON-LD 를 헬퍼로 추출" 이라 주장하지만 3개 라우트 전부 여전히 자체 inline JSON-LD 유지(본 모듈 미참조), 실제 유일 소비자는 `insights/series/[topic]/page.tsx` 뿐이고 `predictions/[date]/page.tsx` 는 동일 이름 함수를 로컬로 포크해 별도 유지 중(stale comment). (4) `debug/factorDeltaStats.ts` 의 anomaly 재귀속 로직(`ts.find(t => t.mean === hit.value)`)이 두 날짜의 mean 이 우연히 같으면 오귀속/중복 가능(PLAUSIBLE, 실측 빈도 미확인 — 별도 스코프로 보류).
- fix: `daily.ts` verify 완료 경로(492행)의 `predictionsGenerated: 0` → `verifiedCount.value ?? 0` 으로 교체(dedup-skip/조기-에러 경로는 verifiedCount 가 애초 undefined 라 결과 동일, 변경 없음). 회귀 테스트 1건 추가(`pipeline-daily.test.ts` — verify 정상 완료 시 `pipeline_runs.predictions` 가 0 이 아님을 검증). `tabpfn-import.ts`/`seo/json-ld.ts` stale 주석 2건 정정.
- `pnpm --filter @moneyball/kbo-data test` 93/93파일 1224/1224테스트 green(+1테스트), `pnpm --filter moneyball test` 581/581파일 4527/4527테스트 green, type-check/lint clean(양쪽 패키지), pre-push green. commit f0acc24c, R4 직push.
- 다음 사이클 추천 = review-code(heavy) 계속(factorDeltaStats 재귀속 로직 실측 검증 또는 json-ld.ts predictions/[date] fork 통합 검토) 또는 gap-fill 자연 대기.

## v0.5.62.204 — 2026-09-03 (cycle 2829, review-code(heavy): elo-history parseTextBlock + wayback elo dead field 제거 SUCCESS)

### review-code(heavy): backtest 영역 신규 8파일 감사 (cycle 2829, SUCCESS)
...
- 방법론 보정: git 1-commit heuristic 만으론 "미감사" 판별 불가 확인 — 과거 CLEAN 판정 파일은 fix 커밋이 없어 git history 1-commit 그대로 남음. CHANGELOG.md/TODOS.md 텍스트 교차검증 병행해 진짜 신규 후보(`backtest/elo-history.ts`/`backtest/logistic.ts`/`backtest/wayback-team-stats.ts`/`pipeline/final-reasoning.ts`/`pipeline/backfill-weather.ts`/`scrapers/kbo-pitcher.ts`/`scrapers/naver-schedule.ts`/`scrapers/umpire.ts`) 8개 확보.
- general-purpose 서브에이전트 전수 감사: 6/8 CLEAN, 2건 발견 — (1) `backtest/elo-history.ts`의 `parseTextBlock()` export가 프로덕션 무호출(`parseEloHistory`가 자체 inline regex 사용, 유일 소비자는 자기 자신의 단위테스트) — 죽은 export. (2) `backtest/wayback-team-stats.ts`의 `SeasonTeamStat.elo` 필드가 파싱만 되고 `loader.ts::buildFeatures()`가 미소비(백테스트 Elo source of truth = `elo-history.ts`의 일별 시계열 `getEloAt`, wayback 시즌 스냅샷 아님).
- fix: `parseTextBlock` 제거(본체+barrel export+고아 테스트) + wayback `elo` 필드 제거(행 유효성 검증용 local var는 유지, 주석 추가로 의도 명시).
- `pnpm --filter @moneyball/kbo-data test` 93/93파일 1224/1224테스트 green, type-check/lint clean, pre-push green. commit 9fe3dc88, R4 직push.
...
- 진단: open issue 0, unprocessed plan 0/23(approved 없음). gap trigger 4종 전부 미도달(fix-incident 11/20, op-analysis 8/25, info-arch 29/30, lotto 16/30). 직전8 distinct=3(review-code(heavy) 6 + polish-ui 1 + operational-analysis(lite) 1) — 2-chain lock 미충족. explore-idea saturation(14/15) 재도달했으나 cycle 2766/2767 과 동일 negative(신규 organic signal 없음, 재확인 생략). IA 30-cycle checkpoint(cycle 2739)가 "다음 재도달은 cycle 2769"라 명시 — 이번 사이클(2768)엔 아직 이름. review-code(heavy) 직전 carry-over(cycle 2767) 는 전부 소진됐으나, cycle 2767 에서 방금 감사한 MLB `buildMlbTeamProfile.ts`/`buildMlbMatchupProfile.ts` 의 KBO 대응 원본(`buildTeamProfile.ts`/`buildMatchupProfile.ts`)이 이 축으로 미검증 상태 — 신규 후보로 선택.
- general-purpose 서브에이전트 전수 감사: select 컬럼 전부 소비 확인, 코멘트/JSDoc 전부 정확(과잉주장 없음). 2건 발견 — (1) `buildMatchupProfile.ts` `Row.hasPrediction` 필드가 계산만 되고 어디서도 안 읽힘(is_correct != null 체크로 이미 대체된 dead field), (2) `MatchupProfile` 의 streak/avgMargin/recentRecord/blowout/closeGame/homeAwayEdge 6개 필드는 dead 아니나(summary 문장에 합쳐져 실제 렌더) team 페이지가 동일 stat 을 개별 라인으로 렌더하는 방식과 표시 방식이 달라 미래 오해 소지.
- fix: `hasPrediction` 필드 제거(선언+할당). `MatchupProfile` 인터페이스에 표시 방식 차이 설명 주석 추가(필드 유지 — 실제 사용 중이므로 삭제 대상 아님).
- tsc/eslint/test 전부 green (571파일 4491건, 필드 제거만 — 신규 테스트 없음). direct main push(c7723ffc).
- 다음 사이클 추천 = IA 30-cycle 재도달(cycle 2769) 자연 발화 대기, 또는 review-code(heavy) 계속 시 미감사 대형 파일(`buildAccuracyData.ts` 767줄/`buildTeamProfile.ts` 잔여 축/`fancy-stats.ts` 545줄) 재탐색.

## v0.5.62.200 — 2026-09-02 (cycle 2766, polish-ui(2-chain lock fallback): Motion 토큰 과잉주장 문서 발견 + 미소비 easing 토큰 제거 SUCCESS)

### polish-ui(2-chain lock fallback): globals.css motion 토큰 감사 (cycle 2766, SUCCESS)

- 진단: open issue 0, unprocessed plan 0/29(approved 없음). gap trigger 4종 전부 미도달(fix-incident 9/20, op-analysis 6/25, info-arch 27/30, lotto 14/30). 직전8 distinct=2(review-code(heavy) 7 + operational-analysis(lite) 1) — 2-chain alternation lock 발동, 둘 다 후보 제외. explore-idea saturation(12/15) 도달했으나 4-source 재확인 negative(TODOS Next-Up stale, GH issue 0, plan#29 트래픽 count 무성장 지속, 신규라우트 -7d 는 git 이력상 실제 추가 0건=mtime 부작용) — 잔여 chain 전부 organic trigger 부재 → polish-ui 강제 발화(lock rule).
- DESIGN.md token vs 실제 컴포넌트 grep 계열 탐색 — `globals.css` Motion 섹션 토큰 6개(`--motion-fast/medium/slow`, `--ease-out/in/in-out`) 전수 실사용처 확인. `--motion-medium`/`--motion-slow`/`--ease-in`/`--ease-in-out` 4개가 CSS 어디에도 `var()` 로 실제 적용된 곳 없음(테스트 assertion·주석·JS 근사치 계산만 존재).
...
- `PredictReveal.test.tsx` 에 lock-in 테스트 1건 추가(transition 규칙 실존 + `--ease-in`/`--ease-in-out` 부재 assert). tsc/eslint/test 전부 green (571파일 4491건, +1). direct main push.
...
- 진단: open issue 0, unprocessed plan 0/23(전부 completed/archived/deferred/tier4/completed_*). 직전8 distinct=4(review-code(heavy)4+polish-ui2+fix-incident1+info-architecture-review1) — 2-chain lock 미충족. fix-incident gap 3/20·op-analysis gap 13/25·info-arch gap 1/30 전부 미도달. lotto gap 51/30 재확인 결과 노이즈(cron 이 9/5 picks + 8/29 result 파일 둘 다 당일 자동 갱신). explore-idea saturation 3/15 미충족. gh run list 최근 10건 실패 0(CI Failure/Vercel Deploy Failure Dispatch 전부 skipped). skill-evolution 마커 없음, milestone(2740%50=40) 미도달 — review-code(heavy) 자연 재개(cycle 2739 추천대로).
- elo-trend 5파일 패밀리(standings/buildEloTrend, teams/buildTeamEloTrend, matchup/buildMatchupEloTrend, mlb/buildMlbMatchupEloTrend, mlb/buildMlbTeamEloTrend) 전수감사 — 전부 clean(select 필드 전부 소비, avg 계산용 전체 팀 fetch 는 의도된 설계).
- 인접 축 재점검: `prediction_type` select-only-unused 컬럼 제거 패밀리(cycle 2714 analysis-data.ts 3곳)가 매치업 페이지 전용 팀 팩터 평균 쿼리 2곳(`buildTeamFactorAverages.ts`/`buildMlbTeamFactorAverages.ts`)엔 아직 미적용 확인 — 둘 다 `.eq("prediction_type", "pre_game")` 서버측 필터는 이미 걸려있는데 select 절에도 중복 포함, PredRow 인터페이스에도 필드 정의만 있고 perspectives 추출 루프에서 전혀 안 읽힘.
- fix: 두 파일 모두 select 절 + interface 에서 `prediction_type` 제거(필터 자체는 유지 — cycle 2678/2680/2714 precedent 로 select 없이도 embedded `.eq()` 필터 정상 동작 확인됨).
- tsc/eslint clean, 전체 테스트 572파일 4491건 green(회귀 없음, 순증 0 — 필드 제거만이라 기존 테스트로 충분 커버).
- 다음 사이클 추천 = review-code(heavy) 신규 대상 계속(예: buildTeamRecentForm/buildTeamUpcoming/buildMatchupUpcoming KBO+MLB 페어) 또는 op-analysis(gap 14/25 monitor).
...
- carry-over(cycle 2721) 미감사 잔여 2개 중 `compareModels.ts`(309줄) 서브에이전트 위임 전수감사 — `ModelGroupStats.correctCount`(`aggregateByModel()`이 계산+반환하지만 유일 소비처 `debug/model-comparison/page.tsx`는 `.accuracy`만 읽음, `compareModels.test.ts`/`compareModels-shadow.test.ts`도 미참조) computed-but-unconsumed 발견 + 제거. 내부 누적기 `g.correct`는 accuracy 계산에 계속 사용되어 유지. JSDoc 전체 검증(Brier 계산/KST 버켓팅/calibration bucket 경계/shadow-row LLM_DEBATE_VERSION scoping/정렬 순서) — stale comment 0건.
- tsc/eslint clean, 전체 테스트 571파일 4483건 green. main 직접 커밋 + push (`be4e2412`).
- computed-but-unconsumed 패턴 재발(cycle 2661~2722 계열 연장).
- 다음 사이클 추천 = review-code(heavy) rotation 계속(`hub-dispatch.ts` 잔여 1개) 또는 info-architecture-review(gap 14/30).

### review-code(heavy): buildSeasonSummary.ts 전수감사 clean (cycle 2721, RETRO-ONLY)
...
- carry-over(cycle 2720) 미감사 잔여 3개 중 `buildSeasonSummary.ts`(365줄) 전수감사 — `SeasonSummary` 전 필드(teams/byMonth/championship/topTotalRuns/topMargin/lowTotalRuns 등) `seasons/[year]/page.tsx` 소비 교차검증 — computed-but-unconsumed/stale-comment 계열 0건. 한국시리즈 동점 tie-break는 기존 fix 반영 상태 확인(신규 이슈 아님).
- 코드 변경 없음.
...
- carry-over(cycle 2717/2718) 미감사 대형파일 목록 중 최대 `factor-explanations.ts`(416줄) 서브에이전트 위임 전수감사.
- `FactorRawDetails`(18필드)/`FactorExplanation`(9필드)/`GameOverviewInput`(16필드)/상수 2개/함수 2개 전부 repo 전체 grep 교차검증 — computed-but-unconsumed/select-only-unused/declared-but-never-computed/stale-comment 패턴 0건.
- `analysis-data.ts:702`가 `buildGameOverview()` 반환값 중 `tags`를 discard하는 지점 발견했으나 리스트뷰(summary만 렌더)/상세뷰(tags 렌더) 분리는 의도된 설계로 확인 — 버그 아님.
- 코드 변경 없음.
...
### explore-idea(lite): plan#29 재확인 + postseason 시기상조 판정 (cycle 2718, RETRO-ONLY)

- 진단: 2-chain alternation lock 탐지(직전8 = review-code(heavy) 7 + operational-analysis 1, distinct=2) → 둘 다 후보 제외. fix-incident gap trigger 도달(직전20 0회)했으나 실측 무이슈(`/api/health` overall=ok, `gh run list` 10/10 success/skipped, debug 커밋 0건). explore-idea saturation 14/15(역대 최고 근접) 동시 충족 → explore-idea(lite) 자연 선택.
...
- 신규 리드 스캔: `/mlb/postseason`·`/mlb/wild-card` ETA "2026-09 라이브 통합" 발견 — 실제 조건은 "정규시즌 종료 직후"(9월 말~10월 초)이며 오늘(9/1)은 시즌 진행 중이라 포스트시즌 대진표 자체 미확정, 착수 시기상조로 보류.
- info-arch(cycle 2709 checkpoint 재확인, drift 0)/lotto(cron self-heal 확인)/polish-ui(컴포넌트 hex 3건 전부 의도된 사용) 재확인 결과 실질 이슈 없음.
- 코드 변경 없음.
- 다음 사이클 추천 = 2-chain lock cooldown 만료 후 review-code(heavy) rotation 재개 또는 info-architecture-review. MLB postseason 재평가 = 정규시즌 종료 확정 시점 도달 시 최우선 후보.

### review-code(heavy): mlb-shared.ts 미소비 필드 3개 제거 (cycle 2717, SUCCESS)

- 진단: open issue 0, unprocessed plan 0/23. fix-incident gap trigger 도달(직전20 사이클 0회 발화) — pipeline_runs 최근7일(50/50 success, error 0) + `/api/health`(overall=ok) + runtime-error-alert(23/23 routes ok) + deploy-drift-alert(recent push gap<1h 정상) 전수 진단 → 실제 인시던트 부재 확인, review-code(heavy) redirect.
- carry-over(cycle 2713) matchup/team-profile 수렴픽 카드 중복 재검토 — `MatchupConvergencePickRecord`(KBO `TeamCode`) vs `MlbMatchupConvergencePickRecord`(MLB `MlbTeamCode` + i18n `STRINGS` 레이어)는 타입 체계 차이 + 주석에 명시된 의도된 분기로 판정, 강제 공통화는 불필요한 추상화로 기각.
- review-code(heavy) rotation 계속 — `mlb-shared.ts`(421줄, 미감사) 서브에이전트 위임 감사 + 직접 repo-wide grep 검증.
- `MlbWeeklyFactorInsight.factor`/`.direction`(mlb weekly/monthly review 4곳 ko/en 모두 `label`/`correlation`/`directionalAccuracy`만 렌더, 미소비) + `MlbMissFactorSupport.supportMagnitude`(내부 정렬키로만 사용, 공개 인터페이스 노출 불필요 — sort 용 내부 intersection 타입으로 분리) 3필드 제거(commit f8a70baa).
- fix: tsc clean + eslint clean + 전체 테스트 571파일 4483건 green + pre-push hook 통과.
