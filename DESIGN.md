...
- **Dark mode (Hybrid C):**
  ...
  - Border: #1e2b23 (`--color-border`, 다크모드 전용 토큰 — 라이트모드는 Tailwind `border-gray-200` literal 사용, 다크모드만 `dark:border-[var(--color-border)]` 로 그린 틴트). **예외**: 반투명 배경/오버레이 위 보더는 `dark:border-gray-N/opacity` 형태 유지 (불투명 토큰 사용 시 배경과 대비 과다) — 8곳 role 목록은 Decisions Log 2026-08-26(cycle 2625) 참조
...
| 2026-08-26 | 잔여 3곳 드롭다운 패널 border-radius `rounded-md`(6px) → `rounded-lg`(8px, md tier) 최종 확정 (silent drift 완전 종료, cycle 2605 review-code heavy) | cycle 2604 가 tier 불명확으로 범위 제외한 `MegaMenu.tsx` 드롭다운 패널 + `navigation-menu.tsx` viewport + `SearchClient.tsx` listbox-option row 3곳 twin 확정. border+shadow-lg 플로팅 패널 계열(`PWAInstallButton`/`QuantOnlyBadge`/`WeeklyTrendMini`/`FactorWaterfallChart`/`MlbFactorWaterfallChart`) 전부 `rounded-lg` 사용 — MegaMenu·navigation-menu 두 곳이 유일한 이탈이자 서로 twin(동일 "대형 드롭다운" 역할)이었음이 확인되며 drift 확정. SearchClient row 는 `HistoricalAnalogMatchup`/`MlbHistoricalAnalogMatchup` 의 `rounded-lg` interactive-row twin 과 정렬. rounded-md drift family 완전 소진(cycle 2599 typography → 2602 뱃지 → 2603 알림박스 → 2604 버튼/인풋/캘린더 → 2605 드롭다운 패널, 전역 grep 0건). `silent-drift-cycle-2604.test.ts` 의 "deferred files still use rounded-md" assertion 이 obsolete 화 되어 제거(전역 clean 단일 assertion으로 통합) + `silent-drift-cycle-2605.test.ts` 신규(3 tests). |
