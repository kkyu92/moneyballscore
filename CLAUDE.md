...
- Migrations (prod 모두 적용):
  ...
  - `020_sp_confirmation_log.sql` — **(2026-04-27)** SP 확정 시각 측정 로그. Cloudflare Worker 가 매 trigger 마다 KBO API B_PIT_P_NM/T_PIT_P_NM 상태 적재. Phase 3 cron 정밀 축소 결정 데이터.
- CI / Cron:
  ...
  - `cloudflare-worker/` **(2026-04-27 신규)**: Cloudflare Workers 기반 cron + SP 측정. (1) Phase 1 — 기존 daily-pipeline schedule 그대로 옮김 + mode 명시 호출 (UTC hour 의존하지 않고 worker 가 결정). (2) Phase 2 — 매 trigger 마다 KBO API 호출 → `sp_confirmation_log` 적재. 1~2주 누적 후 Phase 3 에서 cron 횟수 정밀 축소 (15회 → 4~6회). 무료 tier (Workers Free 100k req/day). Setup: `cloudflare-worker/README.md`.
  - `.github/workflows/pat-expiry-check.yml` / `vercel-deploy-error-dispatch.yml` — worker-incident dispatch
  - `.github/workflows/submit-lesson.yml` — worker-lesson dispatch
- UI:
