...
- **v0.5.18-21 추가** (2026-04-19, AdSense 심사 대비 + 사용자 리텐션 + 운영 안정):
  ...
  - `apps/moneyball/src/app/leaderboard/LeaderboardClient.tsx` — /leaderboard tab persistence (cycle 681, PR #970). saturation v3 후보 E carry-over closure. useState → useSyncExternalStore + localStorage `mb_leaderboard_tab_v1`. SSR safe (getServerSnapshot='weekly' default = 기존 useState default 동일).
  ...
  - `apps/moneyball/src/components/predictions/FactorBreakdown.tsx` — factor 분해 시각화 (169 line, 2026-05-19, cycle 690 phase). pre_game factors 가중치 + bias 막대 그래프 + 0% factor 표기 제외.
