...
- CI / Cron:
  ...
  - `.github/workflows/submit-lesson.yml` — worker-lesson dispatch. **(2026-04-29 Phase 4a D4)** `lesson:` + `policy:` / `feedback:` / `memory:` 4 prefix 모두 허브 worker-lesson 채널로 dispatch. lesson 외 prefix 는 payload `subtype=self-policy` 표시.
  - `.github/workflows/self-develop.yml` **(2026-04-29 Phase 5a 신규 + 비전 1 보완)** — workflow_dispatch + self-hosted [home] runner + claude-code-action@v1. cloudflare-worker daily cron (`0 0 * * *` = KST 09:00) 이 fire. 진단 (gh issue/TODOS/git log) → 결정 (auto/issue/skip) → 실행 (4 prefix commit + PR / human-needed Issue / skip). **10 fire = 1 closed cycle, carry-over Issue 기반 자율 chain** (label `agent-loop,handoff`). **사용자 직접 작업과 namespace 분리** — 모든 자동 결과는 `agent-loop` label + `agent-loop/` branch prefix. 비용 $0 (CLAUDE_CODE_OAUTH_TOKEN 구독 OAuth). repo secrets 필요: `CLAUDE_CODE_OAUTH_TOKEN`. cloudflare-worker secret 필요: `GH_DISPATCH_PAT` (gh auth token 재사용 OK, scope = workflow). repo settings 필요: Actions → General → Workflow permissions → Read+Write + "Allow GitHub Actions to create and approve PRs" ON.
- UI:
