...
- CI / Cron:
  ...
  - `.github/workflows/self-develop.yml` **(2026-04-29 Phase 5a 신규)** — workflow_dispatch + self-hosted [home] runner + claude-code-action@v1. cloudflare-worker daily cron (`0 0 * * *` = KST 09:00) 이 fire. 진단 (gh issue/TODOS/git log) → 결정 (auto/issue/skip) → 실행 (4 prefix commit + PR / human-needed Issue / skip). 10 fire = 1 closed cycle. 비용 $0 (CLAUDE_CODE_OAUTH_TOKEN 구독 OAuth). secrets 필요: `CLAUDE_CODE_OAUTH_TOKEN`. cloudflare-worker secret 필요: `GH_DISPATCH_PAT` (PLAYBOOK_PAT 재사용 OK, scope = actions:write).
- UI:
