...
**Agents reasoning insights series 박제 — plan #3 Step 1~8 closure (cycle 844~856 phase, 2026-05-21, PR #1200~#1213)**:
...
- `apps/moneyball/src/app/insights/[date]/page.tsx` (cycle 847 PR #1203, 206 line) — **Step 3 신규 dynamic 라우트 `/insights/[date]` daily archive Server Component**. `dynamic="force-static"` + `dynamicParams=false` + `revalidate=86400` (ISR 24h) + `generateStaticParams` = `listInsightsDates(90)` 최근 90일 prebuild. 잘못된 date format 또는 entries=0 → `notFound()` (segment-level not-found.tsx 매칭). 일자별 전체 reasoning + 양팀 arg summary 노출 — `JudgeReasoningCard` 재사용 (`homeArgSummary` + `awayArgSummary` + `isQuantOnlyFallback` 양쪽 prop). 각 경기 li 에 `id=game-${gameId}` + `scroll-mt-20` (deep link anchor). 카드 헤더 — `{awayName} vs {homeName}` shortTeamName + statusBadge (cycle 844 hub 패턴 정합) + 홈 승리확률 % + `/analysis/game/${gameId}` link. metadata title=`${date} AI 인사이트` + canonical `${SITE_URL}/insights/${date}` + OG/Twitter `type:"article"` + `publishedTime: ${date}T00:00:00+09:00` + Article JSON-LD per date (`headline` / `description` entries.length 경기 / `datePublished` + `dateModified` = `${date}T00:00:00+09:00`). Breadcrumb wire (홈 → AI 인사이트 → date). 관련 자료 nav 3 chip (← AI 인사이트 hub / 일자 예측 페이지 → / 예측 방법론 →). footer "ISR 24시간 갱신 + 정량 모델 단독 표시 안내" 1 단락.
- `apps/moneyball/src/app/insights/[date]/not-found.tsx` (cycle 847 PR #1203, 70 line) — Next.js 16 segment-level fallback (cycle 799 v13-A 패턴 정합 — 8 dynamic 라우트 not-found.tsx pattern 9번째 합류). 잘못된 date format / 시즌 외 일자 / 미존재 일자 진입 시 fallback. 404 헤더 + 최근 7 일자 nav (오늘/어제 + 5 자연 date) + URL 형식 섹션 (`/insights/YYYY-MM-DD` + ISO 8601) + hub link (← AI 인사이트 hub / 오늘의 예측). `robots: { index: false, follow: false }` metadata.
- `apps/moneyball/src/lib/insights/loader.ts` (cycle 847 PR #1203, 138 line) — Step 3 신규 데이터 로더 모듈. `DATE_REGEX = /^20[2-9]\d-\d{2}-\d{2}$/` + `isValidInsightsDate(date)` (format + month-day range + 윤년 UTC roundtrip 검증). `createInsightsClient()` = `NEXT_PUBLIC_SUPABASE_URL` + `NEXT_PUBLIC_SUPABASE_ANON_KEY` cookieless (`auth: { persistSession: false, autoRefreshToken: false }`) — Server Component build time `generateStaticParams` cookieless 호출 안전. `listInsightsDates(daysBack=90)` = recent N 일 `predictions!inner(games.game_date)` select + `prediction_type='pre_game'` + `games.game_date >= sinceStr` + LIMIT N*6 over-fetch + Set 중복 제거 + sort desc. `getInsightsForDate(date)` = 단일 일자 `predictions!inner(games)` select + `prediction_type='pre_game'` + `games.game_date === date` + `presentJudgeReasoningWithFallback(verdict?.reasoning)` 통과만 누적 + seen Set 으로 game.id 중복 차단. `InsightEntry` interface (gameId / date / homeTeam / awayTeam / status / isCorrect / reasoningText / isFallback / homeArgSummary / awayArgSummary / homeWinProb).
- `apps/moneyball/src/lib/insights/__tests__/loader.test.ts` (cycle 847 PR #1203, 43 line, 6 unit test) — `isValidInsightsDate` regression guard. (1) `2026-05-21` valid / (2) `1999-05-21` invalid (year prefix 20xx 미매칭) / (3) `2026-13-01` invalid (month > 12) / (4) `2026-05-32` invalid (day > 31) / (5) `2025-02-29` invalid (윤년 X) / (6) `2024-02-29` valid (윤년 OK). 신규 6 test PASS — `pnpm test` 510→516.
- `apps/moneyball/src/app/sitemap.ts` — `/insights` URL priority 0.75 daily 추가 (cycle 844, sitemap-warmup cron `'37 * * * *'` ping 대상 + Google Search Console 우선 색인 신호 — cycle 803 `/changelog` 0.6 weekly 대비 `/accuracy` 0.85 daily 다음 비중). **(cycle 847 PR #1203)** `insightsDateRoutes` 17 line 신규 박제 — `listInsightsDates(90)` 최근 90일 동적 URL 생성 + priority 0.7 weekly + lastModified = 각 date `T00:00:00+09:00`. 사용자 가시 URL = `/insights/${date}` 90개 + Google Search Console 색인 신호. `/insights/[date]` ISR 24h + generateStaticParams 90일 prebuild 와 정합 — sitemap URL = 실제 prebuild URL 1:1 매칭.
...
- `apps/moneyball/src/app/insights/twitter-image.tsx` **(cycle 852 PR #1209, 6 line re-export)** — Step 6 박제. `export { default, runtime, size, contentType, alt } from "./opengraph-image"` parity. cycle 817 v13-G twitter-image 8 라우트 parity 패턴 정합 + cycle 794 Turbopack `runtime` re-export silent drift 사례 10 회피 패턴 (statically parsable 강제, default + runtime + size + contentType + alt 명시 박제). twitter-image coverage 9/9 (100%) → 10/10 (100%) — `/insights` hub Twitter card=summary_large_image 노출 박제. 사용자가 `/insights` link 직접 공유 시 rich preview 자동 노출.
- `apps/moneyball/src/app/methodology/page.tsx` **(cycle 853 PR #1210, 1 file 10 insert / 1 delete)** — Step 5 박제. "더 알아보기" footer grid 3-col → 4-col 확장 (`/accuracy` + `/glossary` + `/changelog` + **`/insights` (4번째 신규)**). cycle 822 PR #1181 `/changelog` Header NAV 패턴 정합 — 신규 라우트 박제 시 hub footer cross-link 박제. AdSense 심사 inbound link 강화 — methodology page article surface 에서 /insights timeline 으로 1 클릭 이동 path 박제.
- `apps/moneyball/src/components/layout/Header.tsx` **(cycle 854 PR #1211, 1 file 1 insertion)** — Step 7 박제. NAV 도움말 group `/insights` "AI 인사이트" link 추가 (`/glossary` 다음 + `/changelog` 앞 — Footer SITEMAP "도움말" column 순서 정합). cycle 822 PR #1181 `/changelog` Header NAV 패턴 정합 (cycle 687 PR #975 NAV 도움말 group + cycle 822 PR #1181 /changelog 추가 + 본 cycle 854 PR #1211 /insights 추가 = 3 layer 정합 박제). Header NAV 도움말 group 가시 카탈로그 = /glossary + /insights + /changelog + /about + /contact + /faq.
- `apps/moneyball/src/app/feed/route.ts` **(cycle 855 PR #1212, 1 file 18 line)** — Step 4 박제. RSS feed `<item>` array 에 `/insights/<date>` daily archive 10건 추가. `listInsightsDates(10)` 단일 query (cookieless supabase select recent 10 daily) → 각 entry title `${date} AI 인사이트` + link `${SITE_URL}/insights/${date}` + guid (link 동일, isPermaLink="true") + description "{date} KBO 경기 AI 토론 정량+정성 reasoning timeline" + pubDate (date `T23:59:00+09:00` KST). 위치 = changelog items 10건 다음, review items / game items 와 동일 RSS feed body 안. cycle 810 v13-E `/changelog` entries RSS item 패턴 정합 (cycle 803 /changelog 페이지 + cycle 810 RSS entries + cycle 815 /feed UI link + 본 cycle 855 /insights RSS entries = 5 layer 정합 박제). RSS 구독자 (Feedly / Inoreader / NetNewsWire) 에 일자별 AI 인사이트 즉시 전달.
- `apps/moneyball/src/app/insights/__tests__/insights-routes.test.ts` **(cycle 856 PR #1213, 172 line, 19 신규 unit test)** — Step 8 박제. plan #3 8 Step 박제 regression guard — (1) `/insights` hub page 5 test (404 X / 30 entries 누적 / fallback chip render / breadcrumb wire / Article JSON-LD shape) / (2) `/insights/[date]` daily archive 7 test (valid date 200 / invalid date notFound / dynamicParams=false / Article JSON-LD per date / scroll-mt anchor / JudgeReasoningCard arg summary 양쪽 prop / canonical URL) / (3) `/insights/[date]/not-found.tsx` 2 test (404 metadata + robots noindex) / (4) RSS /feed insights entries 3 test (10 daily 추가 / title-link-pubDate shape / cycle 855 sibling items 정합) / (5) twitter-image parity 2 test (re-export shape + alt). 또한 PR #1213 안에 **feed/route.ts 사례 12 family fix** — cycle 849 PR #1205 사례 12 (games.home_team_code 컬럼 부재 fix) family 의 feed/route.ts 안 남은 query reference 후속 sync. `pnpm test` 510 → 529 (19 신규 PASS).
- 직전 reasoning archive coverage = `/predictions/[date]` 안 `JudgeReasoningCard` (cycle 690 phase) only — 각 경기 카드 안 reasoning 노출 + thin content 회피 + SEO 본문 확보. **시계열 archive / hub 부재** silent drift. cycle 844 phase 가 in-site reasoning timeline hub (`/insights`) 박제 — 사용자 1 페이지에서 최근 30 reasoning 비교 + 1 클릭 anchor 진입. **cycle 847 phase 가 daily archive (`/insights/[date]`) 박제** — 90 daily indexable URL + 일자별 전체 reasoning + 양팀 arg summary 노출 (Article JSON-LD per date + segment-level not-found cycle 799 v13-A 패턴 9번째 합류). **cycle 852~856 phase (5 cycle sweep momentum) 가 plan #3 잔여 5 Step 모두 closure** — twitter-image parity (Step 6 cycle 852) + methodology footer link (Step 5 cycle 853) + Header NAV link (Step 7 cycle 854) + RSS /feed entries (Step 4 cycle 855) + unit test 박제 + feed regression fix (Step 8 cycle 856). AdSense article surface 다층 — hub timeline (Article JSON-LD 1) + daily archive (Article JSON-LD 90 일자별) + RSS /feed entries (10 일자별 RSS reader 채널) + Header NAV + methodology footer + Twitter card 양쪽 확장. **plan #3 8/8 Step closure** (Step 1 hub + Step 2 OG image + Step 3 daily archive + Step 4 RSS entries + Step 5 methodology footer link + Step 6 twitter-image parity + Step 7 Header NAV link + Step 8 unit test + feed fix). plan #1 MLB demand test (Step 1~5+7 완료, Step 6 사용자 영역 carry-over) + plan #2 lotto methodology (Step 1~8 완료, Step 0 사용자 영역 carry-over) + **plan #3 8/8 closure** = 본 메인 자율 영역 closed loop 3 plan 모두 ship 마감. 잔여 carry-over = 사용자 영역 (plan #1 Vercel KV rate limit env + plan #2 AdSense pre-check 14일 monitor).

**Agents reasoning 시즌 2 박제 — plan #5 Step 1~6 closure (cycle 872~875 phase, 2026-05-22, PR #1228~#1230 + cycle 875)**:
- `apps/moneyball/src/lib/insights/loader.ts` **(cycle 873 PR #1229, Step 2~3)** — `InsightEntry` interface 안 `factors: Record<string, number> | null` field 확장. supabase select clause `"is_correct, reasoning, factors, prediction_type, created_at, games!inner(...)"` 안 factors column 추가. row 매핑 시 `rawFactors && typeof rawFactors === "object" && Object.keys(rawFactors).length > 0` null-safe normalize — 빈 객체 / null / 비객체 모두 `factors: null` 통일. predictions.factors JSONB column (cycle 287 박제) source. cookieless supabase client 패턴 정합 (cycle 847 loader 박제).
- `apps/moneyball/src/app/insights/[date]/page.tsx` **(cycle 873 PR #1229, Step 3)** — `<FactorBreakdown factors={item.factors} homeTeam={item.homeTeam} awayTeam={item.awayTeam} />` JudgeReasoningCard 다음 stacked render. `item.factors && (...)` early guard — factors 부재 시 자동 skip (FactorBreakdown 내부 early return null 정합). `scroll-mt-20` deep link anchor 유지. ISR 24h `revalidate=86400` 유지.
- `apps/moneyball/src/app/insights/page.tsx` **(cycle 874 PR #1230, Step 4)** — `/insights` hub 안 timeline entry 카드 footer 에 mini factor preview. InsightRow interface 안 `factors: Record<string, number> | null` 확장. `selectTopFactors(factors, 3)` helper — `Math.abs(value - 0.5)` 거리 desc 정렬 + 상위 3 factor 슬라이스 + `pct = Math.round(value * 100)` 정수 % + `favorable` 분류 (NEUTRAL 0.45~0.55 경계 — home / away / neutral). `data-mini-factor-preview` attr + `topFactors.length > 0` 조건 render. "전체 팩터 보기" anchor link → `/insights/${item.date}#game-${item.gameId}` daily archive deep link. 30 timeline entry 누적 = 최대 90 factor preview render.
- `apps/moneyball/src/lib/insights/topFactors.ts` **(cycle 875 Step 5, 신규 36 line)** — `selectTopFactors` helper 추출. page.tsx inline 함수 → shared utility module. `TOP_FACTOR_LIMIT = 3` + `NEUTRAL_LO = 0.45` + `NEUTRAL_HI = 0.55` constant export. `TopFactor` interface (key + label + pct + favorable) export. FACTOR_LABELS 매핑 키 + `Number.isFinite(value)` 양쪽 filter (unknown_key / NaN / Infinity 차단). unit test 분리 가능 + 재사용 가능 구조.
- `apps/moneyball/src/app/insights/__tests__/insights-routes.test.ts` **(cycle 875 Step 5, +5 신규 behavior test)** — 기존 30 grep regression test + 5 신규 selectTopFactors behavior test = 34 PASS. (1) null factors → 빈 배열 / (2) 빈 객체 + FACTOR_LABELS 미매칭 키 + 비정상 값 (NaN / Infinity) 모두 filter / (3) abs(value - 0.5) 거리 desc 정렬 + TOP_FACTOR_LIMIT=3 슬라이스 + 정렬 순서 검증 (bullpen_fip 0.85 > lineup_woba 0.15 > elo 0.7) / (4) favorable home / away / neutral 분류 경계 (0.7 → home / 0.3 → away / 0.5 → neutral) / (5) pct = Math.round(value * 100) 정수 박제 (0.723 → 72 / 0.314 → 31). `pnpm test` 549 → 554 (regression 0) + `pnpm lint` 0 warning + `pnpm exec tsc --noEmit` 0 error + `pnpm build` /insights + /insights/[date] static prerender 통과.
- 직전 /insights series coverage = JudgeReasoningCard (정성 reasoning + 양팀 arg summary) only — `FactorBreakdown` (184 line, cycle 690/756 박제, glossary inline link) 미통합 silent gap. plan #5 phase 가 정량 (factor breakdown) + 정성 (judge reasoning) 양쪽 archive timeline 박제 — 사용자 1 페이지에서 "왜 이 예측인가" (정성) + "어떤 factor 가 가중치 얼마였나" (정량) 동시 비교 path 박제. AdSense article surface 강화 — factor 한국어 label + percentage 자연어 노출 = thin content 회피 더 강력. glossary inline link (cycle 756 박제) inbound 자연 확장. **plan #5 6/6 Step closure** (Step 1 plan write + Step 2~3 loader factors + integration + Step 4 hub mini preview + Step 5 unit test 분리 + Step 6 documentation sync). plan #3 8/8 closure (cycle 856 박제) + plan #5 6/6 closure (cycle 875 박제) = /insights series 시즌 1+2 본 메인 자율 영역 closed loop 마감. 잔여 carry-over (옵션) = FactorBreakdown anchor support `id="factor-breakdown-${gameId}"` deep link + OG image factor preview overlay (사용자 자연 발화 또는 다음 saturation series fire 시점 redirect).
...
- **scoring_rule 성과 (cycle 864 갱신)**: v1.5(16건,75.0%) / v1.6(46건,37.0%) / v1.7-revert(32건,53.1%) / **v1.8(32건,43.8%, Brier 0.2256)** — cycle 775 30건 43.3% → +2건 +0.5pp in 9일. **v1.8 split (cycle 864 op-analysis heavy)**: real-debate (n=10, 50.0%, Brier 0.2988, mean conf 0.436) vs credit-fail (n=22, 40.9%, Brier 0.1924, mean conf 0.418) — real-debate accuracy 우수 (+9.1pp), credit-fail Brier 우수 (low-confidence pick 의 winner-centric Brier 자연 우수 + real-debate 의 confidence calibration 잠재 over-confidence 신호, n=10 작음 추가 측정 필요). n=150 ETA = 531일 (현재 velocity 0.22/day). cycle 775 v1.8 date distribution — 05-13 5/5 real(시즌 첫날) / 05-14 0/5 credit-fail / 05-15 0/5 credit-fail / 05-16 3C+2R / 05-17 4C+1R / 05-19 0/5 credit-fail / 05-21 2R (cycle 864 추가)
...
- `packages/kbo-data/src/pipeline/`:
  ...
  - `silent-drift-alert.ts` **(cycle 819, 2026-05-21 신규, PR #1179 / cycle 864, 2026-05-22 정정, PR #1220)** — silent drift family 사례 11 후속 운영 alert 채널. `SilentDriftAlertMeta` interface (mode + date + gamesFound + predictionsGenerated + errors + **existingPredictionsCount?: number** — cycle 864 추가, optional, 미지정 시 0 fallback 기존 caller 동작 보존) + `shouldAlertSilentDrift(meta)` pure logic (**cycle 864 정정**: `mode==='predict_final' && gamesFound>0 && coverage<gamesFound` where `coverage = predictionsGenerated + (existingPredictionsCount ?? 0)` — 기존 `predictionsGenerated===0` 단순 체크가 predict mode 아침 박제 row 미고려 → 86% false positive). `captureSilentDriftAlert(meta)` 동적 `@sentry/nextjs` import (validator.ts 패턴 정합, `packages/kbo-data` 가 @sentry/nextjs 직접 의존 X 회피). `NODE_ENV==='test'` early return + sentry 부재 silent fallback + capture try/catch silent (main path 보호). 13 unit test (cycle 864 정정 후: 5 신규 = existing=games 미발화 / partial existing 보완 / partial without existing alert / fallback / mixed cover + 2 정정 = "predictionsGenerated>0=OK" 단순 룰 → "coverage=full" 엄밀화 + 6 기존). cycle 813 root cause Layer 1 fix (PR #1173 allowLateWindow) 와 별도 운영 alert Layer 2 — 다음 silent silent drop 발생 시 사용자 가시 metric loss 차단 + cover 한 정상 동작 false alert 차단.
